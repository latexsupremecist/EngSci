...
\question{A very long conductor in free space carrying a current $I$ is parallel to, and at a distance $d$ from, an infinite plane interface with a medium.}
...
\question{An infinitely-long ferromagnetic cylinder of radius $a$ in air has a nonuniform magnetization. In a cylindrical coordinate system whose $z$-axis coincides with the cylinder axis, $\vec M = M_0\frac{r}{a}\hat a_\phi$ ($0 \leq r \leq a$), where $M_0$ is a constant. Find the current densities $\vec J_m$ in the cylinder and $\vec J_{ms}$ on the cylinder, as well as the magnetic flux density $\vec B$ inside and outside the cylinder.}
...
\question{A very large slab of material of thickness $d$ lies perpendicularly to a uniform magnetic field of intensity $\vec H_0 = \hat a_z H_0$. Find the magnetic field intensity inside the dielectric slab if it has a permeability $\mu$ or if it is a permanent magnet having a magnetization vector $M_i = \hat a_z M_i$.}
